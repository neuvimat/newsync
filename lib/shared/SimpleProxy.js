export const proxyKey = Symbol('proxyRef')
export const pristineKey = Symbol('pristineRef')
...
function createLowPrioHandler(level, pristine, dictionary, target, prop, lowPrioChanges) {
  const handler = {set: (key, value) => {
      let shortKey = dictionary.shorten(key)
  ...
  if (dictionary) {
    ...
    recHandler = {
      get(target, prop, receiver) {
        ...
        if (typeof pristine[prop] === 'object' && !(Array.isArray(pristine[prop]))) {
          if (!target[prop][proxyKey]) {
            pristine[prop][proxyKey] = new Proxy(pristine[prop], makeSimpleHandler(changes, [...level, prop], pristine[prop], undefined, lowPrioChanges))
          }
          return target[prop][proxyKey]
        }
        return pristine[prop]
