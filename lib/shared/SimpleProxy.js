 ...
 */
export function makeSimpleRecursiveProxy(dictionary) {
  const changes = {}
  const pristine = {}
  pristine[proxyKey] = true
  const proxy = new Proxy(pristine, makeSimpleHandler(changes, [], pristine, dictionary))
...
function makeSimpleHandler(changes, level = [], pristine, dictionary) {
  let recHandler
  if (dictionary) {
    recHandler = {
      get(target, prop, receiver) {
        ...
        if (typeof pristine[prop] === 'object' && !(Array.isArray(pristine[prop]))) {
          if (!target[prop][proxyKey]) {
            pristine[prop][proxyKey] = new Proxy(pristine[prop], makeSimpleHandler(changes, [...level, prop], pristine[prop],dictionary))
          }
      ...
      set(target, prop, value) {
        target[prop] = value;
        const shortProp = dictionary.shorten(prop)
        let cursor = changes;
        for (let l of level) {
          const short = dictionary.shorten(l)
          if (!cursor[short]) {
            cursor[short] = {}
          }
          cursor = cursor[short]
