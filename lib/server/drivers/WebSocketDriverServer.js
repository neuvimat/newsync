...
export class WebSocketDriverServer extends IDriver {
  ...
  send(object, client) {
    const msgPackMessage = this.neuSync.coder.packr.pack(object)
  ...
  sendToAll(object) {
    ...
    wholeMessage.set(this._prefixBytes, 0)
    wholeMessage.set(msgPackMessage, this._prefixBytes.length)
    const clients = this.neuSync.clients
  ...
  getDataFromMessage(data) {
    return this.neuSync.coder.decode(data.substring(1, data.length))
  }
  ...
  isFrameworkMessage(data) {
    ...
    console.log('isInstanceOfBuffer?', data instanceof ArrayBuffer, typeof data);
    return false
  }
