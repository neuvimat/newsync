...
export class WebSocketDriverServer extends IDriverClient {
  ...
  sendToAllLowPrio(message) {
    // Technically we should not get here because NewSync and any responsible programmer should before use
    // isLowPrioSupported, but just in case
  ...
  scheduleLowPrio(client, message) {
  ...
  sendMessageImmediate(client, message) {
