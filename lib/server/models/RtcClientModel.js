...
export class RtcClientModel extends AbstractClientModel{
  ...
  /**
   * WebRTC only. Attempts to create a new channel that will be used for low priority communication
   * @param channelName
   * @return {Promise<unknown>}
   */
  attemptToCreateLowPrioChannel(channelName = 'NewSyncLowPrio') {
    if (this.lowPrioChannel === undefined) {
      console.log(`No low prio channel exists for client ${this.id}, attempting to create one automatically!`);
