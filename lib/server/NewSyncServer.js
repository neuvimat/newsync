...
export class NewSyncServer {
  ...
  bundleContainersChanges(client, message) {
    ...
    const iterateIn = client.allContainers ? Object.keys(this.containers) : client.whitelist
    for (let k of iterateIn) {
      const shortKey = this.dict.shorten(k)
      if (client.fullUpdate[k]) {
        wasChanged = true
        message[KEYWORDS.containers][shortKey] = this.getFullContainerChanges(k)
  ...
  getContainerChanges(container) {
    // if (!this.containers[container]) {console.error('Trying to get non existing container! ', container); return
    // null;}
  ...
  getFullContainerChanges(container) {
    // if (!this.containers[container]) {console.error('Trying to get non existing container! ', container); return
    // null;}
    if (!this.fullContainersCache[container]) {
      this.fullContainersCache[container] = this.coder.pack(this.dict.shortenObject(this.containers[container].pristine))
    }
