    ...
    for (let k of iterateIn) {
      if (this.lowContainersCache[k] === undefined) {
        if (!isEmpty(this.containers[k].lowPrio)) {
          ...
          msg[INDICES.containers][shortKey] = containerData
          this.lowContainersCache[k] = this.coder.pack(this.dict.shortenObject(containerData))
        }
        else {
  ...
  sync() {
    ...
    for (const c of clientsKeys) {
      ...
      if (this.driver.isLowPrioSupported()) {
        lpMessage = this.getLowPrioChangesMessage()
