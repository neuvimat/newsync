...
import {applyDeletes, applyMeta, clear, isEmpty, merge} from "@Lib/util/objUtil";
...
/**
 * {@link NewSyncServer}, but server side with some limited functionality. Because inheritance was used to share some
 * of the codebase, there are some specific methods that cannot be used on the client. These will throw an exception,
 * however just by thinking rationally it should be obvious which methods it will be.
 */
export class NewSyncClient extends NewSyncServer {
  ...
  /**
   * Extract the dictionary update from the incoming message and apply it.
   * @param message {*}
   */
  ...
  /**
   * Extract container changes from the NewSync message and apply them.
   * @param message {{}}
   * @return {{}} restored containers from the message (i.e. with previous, long keys)
   */
  handleContainers(message) {
    ...
    if (rawContainers) {
      ...
      for (let k in restoredContainers) {
        ...
        this._applyContainerChanges(this.containers[k].pristine, restoredContainers[k])
      }
    }
  ...
  /**
   * Apply extracted changes to the container
   * @param containerState {{}}
   * @param containerChanges {{}}
   */
  _applyContainerChanges(containerState, containerChanges) {
